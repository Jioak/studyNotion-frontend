...
export default function VideoDetailsSlider({setReviewModal}) {
    ...
    const {courseSectionData,courseEntireData,totalNoOfLectures,
...
<div className="h-[calc(100vh - 5rem)] overflow-y-auto">
    {
        courseSectionData.map((section,index)=> (
            ...
            <div onClick={()=>setActiveStatus(section?.id)} key={index}
        ...
        <div  className="transition-[height] duration-500 ease-in-out">
{
    section.subSection.map((topic,index)=>(
        ...
        <div className={`flex gap-5 p-5 px-5 py-2 
        ${videobarActive === topic._id ? 'bg-yellow-200 text-richblack-900':'bg-richblack-900 text-white' }`}
        key={index}
         onClick={()=>{navigate(`/view-course/${courseEntireData?._id}/section/${section?._id}/sub-section/${topic?.id}`)
